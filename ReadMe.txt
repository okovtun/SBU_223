...
DONE:
1. От ветки 'templated' создать ветку 'template_separated', и в ней вынести шаблонные функции в отдельные файлы;
2. Вычислить максимально возможный объем памяти для восьмибайтового адреса;
3. Выучить теорию по указателям!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

DONE:
1. Написать функцию ??? Search(???), которая находит в массиве повторяющиеся значения,
   выводит их на экран, а так же выводит на экран количесто повторений.
   Массив должен быть заполнен заведомо повторяющимися значениями, 
   для этого диапазон генрации случайных чисел должен быть менше размера массива,
   например, размер массива - 10 элементов, случайные числа от 50 до 55.
2. Перегрузить все функции, написанные в проекте Functions\Arrays для одномерных
   и двумерных массивов типа char, int, float, double
...
В Solution Functions добавить проект Arrays, и в этом проекте реализовать следующие функции:
	...
	??? UniqueRand(???);//Заполняет массив уникальными случайными числами		DONE
